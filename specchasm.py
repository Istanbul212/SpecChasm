#!/usr/bin/env python3
"""SpecChasm: Lean-backed mutation-gap analyzer MVP."""
...
def render_lean_source(
    ...
    return render_namespaced_lean_source("SpecChasmGenerated", title, spec, rules, default, outputs)


def render_namespaced_lean_source(
    namespace: str,
    title: str,
    spec: Spec,
    rules: Sequence[Rule],
    ...
    return f"""\
-- Generated by SpecChasm.
{namespace_open}

{render_prelude(spec, outputs)}

{render_check_body(title, spec, rules, default)}

{namespace_close}
"""


def render_check_body(title: str, spec: Spec, rules: Sequence[Rule], default: str) -> str:
    return f"""\
...
def render_namespaced_check_body(
    namespace: str,
    title: str,
    spec: Spec,
    rules: Sequence[Rule],
...
def render_batch_lean_source(blocks: Sequence[CheckBlock], spec: Spec) -> Tuple[str, Dict[str, RenderedCheckBlock]]:
    lines: List[str] = [
        "-- Generated by SpecChasm.",
        "namespace SpecChasmGenerated",
        "",
    ]
    lines.extend(render_prelude(spec).rstrip("\n").split("\n"))
    lines.append("")
    rendered_blocks: Dict[str, RenderedCheckBlock] = {}

    for block in blocks:
        block_outputs = block.outputs if block.outputs is not None else spec.outputs
        if block_outputs != spec.outputs:
            continue
        source = render_namespaced_check_body(
            safe_identifier(block.key),
            block.title,
            spec,
            block.rules,
            block.default,
        )
        block_lines = source.rstrip("\n").split("\n")
        start_line = len(lines) + 1
        lines.extend(block_lines)
        end_line = len(lines)
        rendered_blocks[block.key] = RenderedCheckBlock(
            key=block.key,
    ...
    lines.append("end SpecChasmGenerated")
    lines.append("")
    ...
    for block in blocks:
        ...
        if block_outputs == spec.outputs:
            continue
        source = render_namespaced_lean_source(
            f"SpecChasmGenerated.{safe_identifier(block.key)}",
            block.title,
            spec,
            block.rules,
            block.default,
...
LEAN_ERROR_RE = re.compile(r"^.*?:(\d+):\d+:\s+error:")


def lean_error_line(output_line: str) -> Optional[int]:
    match = LEAN_ERROR_RE.match(output_line)
    if not match:
        return None
    return int(match.group(1))


def lean_errors_in_range(text: str, start_line: int, end_line: int) -> str:
    lines = []
    for line in text.splitlines():
        error_line = lean_error_line(line)
        if error_line is not None and start_line <= error_line <= end_line:
            lines.append(line)
    return "\n".join(lines)


def lean_has_unmapped_error(text: str, rendered_blocks: Dict[str, RenderedCheckBlock]) -> bool:
    for line in text.splitlines():
        error_line = lean_error_line(line)
        if error_line is None:
            continue
        if not any(block.start_line <= error_line <= block.end_line for block in rendered_blocks.values()):
            return True
    return False


def check_from_batch(batch_check: LeanCheck, rendered_block: RenderedCheckBlock, has_unmapped_error: bool) -> LeanCheck:
    stdout = lean_errors_in_range(batch_check.stdout, rendered_block.start_line, rendered_block.end_line)
    stderr = lean_errors_in_range(batch_check.stderr, rendered_block.start_line, rendered_block.end_line)

    if has_unmapped_error and not stdout and not stderr:
        stderr = batch_check.stderr

    ok = not stdout and not stderr
    return LeanCheck(
        ok=ok,
        path=rendered_block.source_name,
        stdout=stdout,
        stderr=stderr,
        returncode=0 if ok else batch_check.returncode or 1,
    )
...
def analyze_spec_data(
    ...
    original_lean = render_lean_source("original model", spec, spec.model, spec.default)
    mutants = generate_mutants(spec)
    blocks = [
        CheckBlock(
            key="Original",
            source_name="Original.lean",
    ...
    blocks.extend(
        CheckBlock(
            ...
            source_name=f"{mutant.mutant_id}_{safe_identifier(mutant.name)}.lean",
            title=mutant.name,
            rules=mutant.model,
            default=mutant.default,
            outputs=mutant.outputs,
        )
        for mutant in mutants
    )

    batch_lean, rendered_blocks = render_batch_lean_source(blocks, spec)
    batch_check = run_lean_source(resolved_lean_bin, "SpecChasmBatch.lean", batch_lean)
    has_unmapped_error = lean_has_unmapped_error(batch_check.stdout, rendered_blocks) or lean_has_unmapped_error(
        batch_check.stderr,
        rendered_blocks,
    )
    original_check = check_from_batch(batch_check, rendered_blocks["Original"], has_unmapped_error)

    mutant_results = []
    for mutant in mutants:
        check = check_from_batch(batch_check, rendered_blocks[mutant.mutant_id], has_unmapped_error)
        survived = check.ok
